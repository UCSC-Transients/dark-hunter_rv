...
Also (08-full): load LAMOST/RAVE (and related) epochs from star ``*_summary.txt``
``[EXTERNAL RV DATA]`` and nearest-BJD join them to the literature master (and to
pipeline epochs when both exist).
...
from darkhunter_rv.gaia_utils import parse_external_rvs_from_star_summary  # noqa: E402
from darkhunter_rv.method_evaluation import exposure_method_flags  # noqa: E402
from darkhunter_rv.summary_paths import parse_object_id_from_summary  # noqa: E402
...
DEFAULT_EXTERNAL_SOURCES: tuple[str, ...] = ("LAMOST_LRS", "LAMOST_MRS", "RAVE_DR6")
...
def load_external_epochs_from_summaries(
    summaries_glob: str,
    *,
    sources: Sequence[str] = DEFAULT_EXTERNAL_SOURCES,
    gaia_ids: Sequence[str] | None = None,
) -> pd.DataFrame:
    """
    Load ``[EXTERNAL RV DATA]`` epochs from star summaries for selected telescope prefixes.

    Parameters
    ----------
    summaries_glob:
        Glob of ``*_summary.txt`` paths (shell-expanded by :func:`glob.glob`).
    sources:
        Telescope name prefixes / exact names to keep (default LAMOST + RAVE).
        Matching is case-insensitive prefix on ``telescope``.
    gaia_ids:
        Optional allow-list of Gaia DR3 ids; if set, skip other summaries.
    """
    wanted = [s.strip().upper() for s in sources if s.strip()]
    allow = {str(g) for g in gaia_ids} if gaia_ids is not None else None
    paths = sorted(glob_paths(summaries_glob))
    rows: list[dict[str, Any]] = []
    for path_str in paths:
        path = Path(path_str)
        gaia_id = parse_object_id_from_summary(path) or parse_gaia_id_from_path(path)
        if not gaia_id:
            continue
        gaia_id = str(gaia_id)
        if allow is not None and gaia_id not in allow:
            continue
        for ext in parse_external_rvs_from_star_summary(path):
            tel = str(ext.get("telescope", "") or "").strip()
            tel_u = tel.upper()
            if wanted and not any(tel_u == w or tel_u.startswith(w) for w in wanted):
                continue
            try:
                mjd = float(ext["mjd"])
                rv = float(ext["rv"])
                err = float(ext.get("rv_err", float("nan")))
            ...
            rows.append(
                {
                    ...
                    "rv_err_kms": err if np.isfinite(err) else np.nan,
                    "teff": float("nan"),
                    "epoch_id": f"external:{tel}:{gaia_id}:{mjd:.5f}",
...
def nearest_literature_join(
    ...
    *,
    ...
    """
    For each left-side epoch, attach the nearest literature epoch by |ΔMJD| on the same Gaia id.

    Parameters
    ----------
    pipeline:
        Rows with ``gaia_dr3_id``, ``mjd``, ``rv_kms``, ``method`` (pipeline or external).
    literature:
...
def nearest_pipeline_join(
    ...
    *,
    ...
    """
    For each external epoch, attach nearest pipeline exposure×method by |ΔMJD| on same Gaia id.

    ΔRV = external − pipeline.
    """
    if external.empty or pipeline.empty:
        return pd.DataFrame()

    pipe = pipeline.copy()
    pipe["gaia_dr3_id"] = pipe["gaia_dr3_id"].astype(str)
    pipe["mjd"] = pd.to_numeric(pipe["mjd"], errors="coerce")
    pipe["rv_kms"] = pd.to_numeric(pipe["rv_kms"], errors="coerce")

    out_rows: list[dict[str, Any]] = []
    for _, erow in external.iterrows():
        gid = str(erow["gaia_dr3_id"])
        emjd = float(erow["mjd"])
        if not np.isfinite(emjd):
            continue
        pg = pipe[pipe["gaia_dr3_id"] == gid]
        pg = pg[np.isfinite(pg["mjd"].astype(float)) & np.isfinite(pg["rv_kms"].astype(float))]
        if pg.empty:
            continue
        deltas = (pg["mjd"].astype(float) - emjd).abs()
        j = int(deltas.to_numpy().argmin())
        prow = pg.iloc[j]
        delta_days = float(deltas.iloc[j])
        if max_delta_days is not None and delta_days > float(max_delta_days):
            continue
        ext_rv = float(erow["rv_kms"])
        pipe_rv = float(prow["rv_kms"])
        out_rows.append(
            {
                "gaia_dr3_id": gid,
                "external_source": str(erow["method"]),
                "external_mjd": emjd,
                "external_rv_kms": ext_rv,
                "external_rv_err_kms": float(erow.get("rv_err_kms", np.nan)),
                "pipeline_method": str(prow["method"]),
                "pipeline_basename": str(prow.get("basename", "")),
                "pipeline_mjd": float(prow["mjd"]),
                "pipeline_rv_kms": pipe_rv,
                "pipeline_rv_err_kms": float(prow.get("rv_err_kms", np.nan)),
                "delta_days": delta_days,
                "delta_rv_kms": ext_rv - pipe_rv,
                "abs_delta_rv_kms": abs(ext_rv - pipe_rv),
                "summary_path": str(erow.get("summary_path", erow.get("diagnostics_path", ""))),
            }
        )
    return pd.DataFrame(out_rows)
...
def per_star_bias_rms(pairs: pd.DataFrame) -> pd.DataFrame:
    """Per (gaia_dr3_id, method) mean bias and RMS of left − literature ΔRV."""
    if pairs.empty:
        return pd.DataFrame(
            columns=[
                "gaia_dr3_id",
        ...
        rows.append(
            {
                ...
                "P_orb_days": g["P_orb_days"].dropna().iloc[0] if "P_orb_days" in g and g["P_orb_days"].notna().any() else np.nan,
                "M_star_msun": g["M_star_msun"].dropna().iloc[0] if "M_star_msun" in g and g["M_star_msun"].notna().any() else np.nan,
                "M2_msun": g["M2_msun"].dropna().iloc[0] if "M2_msun" in g and g["M2_msun"].notna().any() else np.nan,
            }
        )
    return pd.DataFrame(rows)
...
def _md_table(df: pd.DataFrame, float_cols: Sequence[str]) -> list[str]:
    if df.empty:
        return ["_Empty._"]
    hdr = list(df.columns)
    lines = [
...
def _write_report_md(
    ...
    *,
    ...
    diagnostics_glob: str,
    master_path: Path,
    summaries_glob: str | None,
    external_sources: Sequence[str],
    n_external_epochs: int,
    n_external_stars: int,
    ext_lit_pairs: pd.DataFrame,
    ext_per_star: pd.DataFrame,
    ext_pipe_pairs: pd.DataFrame,
    acceptance_notes: Sequence[str],
) -> Path:
    lines = [
        f"# {title}",
        "",
        ...
        f"- Overlap stars (pipeline ↔ literature): **{n_stars}**",
        f"- Nearest-BJD pipeline↔literature pairs: {len(pairs)}",
    ]
    if summaries_glob:
        lines.extend(
            [
                f"- Summaries glob: `{summaries_glob}`",
                f"- External sources: {', '.join(external_sources)}",
                f"- External epochs loaded: {n_external_epochs}",
                f"- Stars with external↔literature pairs: **{n_external_stars}**",
                f"- External↔literature pairs: {len(ext_lit_pairs)}",
                f"- External↔pipeline pairs: {len(ext_pipe_pairs)}",
            ]
        )
    lines.extend(["", "## Acceptance / coverage", ""])
    lines.extend(f"- {n}" for n in acceptance_notes)
    lines.extend(["", "## Per-star bias / RMS (pipeline − literature)", ""])
    lines.extend(
        _md_table(
            per_star,
            ("bias_kms", "rms_kms", "median_abs_delta_kms", "median_delta_days"),
        )
    )
    if summaries_glob:
        lines.extend(["", "## Per-star bias / RMS (external − literature)", ""])
        lines.extend(
            _md_table(
                ext_per_star,
                ("bias_kms", "rms_kms", "median_abs_delta_kms", "median_delta_days"),
    ...
    if summaries_glob:
        lines.extend(
            [
                "- `external_literature_pairs.csv` — nearest literature epoch per LAMOST/RAVE row",
                "- `external_per_star_bias_rms.csv` — bias/RMS for external − literature",
                "- `external_pipeline_pairs.csv` — nearest pipeline epoch per external row",
            ]
        )
    lines.extend(
        [
            "",
            "Epoch separations may be large when APF/literature/catalog campaigns do not overlap",
            "in time; nearest-BJD join still runs (optionally capped with `--max-delta-days`).",
            "",
        ]
    )
...
def run_compare(
    *,
    ...
    summaries_glob: str | None = None,
    external_sources: Sequence[str] = DEFAULT_EXTERNAL_SOURCES,
    restrict_external_to_literature: bool = True,
) -> dict[str, Any]:
    """Load diagnostics (+ optional summaries), nearest-BJD join, write report artifacts."""
    literature = load_literature_epochs(master_path)
    lit_ids = sorted(set(literature["gaia_dr3_id"].astype(str))) if not literature.empty else []
    ...
    ext = pd.DataFrame()
    ext_lit = pd.DataFrame()
    ext_per_star = pd.DataFrame()
    ext_pipe = pd.DataFrame()
    if summaries_glob:
        allow = lit_ids if restrict_external_to_literature else None
        ext = load_external_epochs_from_summaries(
            summaries_glob,
            sources=external_sources,
            gaia_ids=allow,
        )
        ext_lit = nearest_literature_join(ext, literature, max_delta_days=max_delta_days)
        ext_per_star = per_star_bias_rms(ext_lit)
        ext_pipe = nearest_pipeline_join(ext, pipeline, max_delta_days=max_delta_days)
        # Merge orbit params for stars only seen via external
        orbit_ext = orbit_qa_table(ext_lit)
        if not orbit_ext.empty:
            if orbit.empty:
                orbit = orbit_ext
            else:
                orbit = (
                    pd.concat([orbit, orbit_ext], ignore_index=True)
                    .drop_duplicates(subset=["gaia_dr3_id"])
                    .sort_values("gaia_dr3_id")
                    .reset_index(drop=True)
                )

    n_stars = int(per_star["gaia_dr3_id"].nunique()) if not per_star.empty else 0
    n_ext_stars = int(ext_per_star["gaia_dr3_id"].nunique()) if not ext_per_star.empty else 0
    acceptance_notes = [
        f"Pipeline↔literature unique El-Badry stars: {n_stars} (full acceptance target ≥10).",
        f"Published orbit columns present on orbit_qa rows: "
        f"{'yes' if (not orbit.empty and 'P_orb_days' in orbit.columns) else 'no'}.",
        f"LAMOST/RAVE summary stars with literature pairs: {n_ext_stars}.",
    ]
    if n_stars < 10:
        acceptance_notes.append(
            "PARTIAL: diagnostics on disk cover <10 El-Badry stars with APF epochs; "
            "need more pipeline reductions of literature cohort before full acceptance."
        )
    ...
    if summaries_glob:
        ext.to_csv(report_dir / "external_epochs.csv", index=False)
        ext_lit.to_csv(report_dir / "external_literature_pairs.csv", index=False)
        ext_per_star.to_csv(report_dir / "external_per_star_bias_rms.csv", index=False)
        ext_pipe.to_csv(report_dir / "external_pipeline_pairs.csv", index=False)

    title = "Literature RV cross-check (full)" if summaries_glob else "Literature RV cross-check (lite)"
    _write_report_md(
        report_dir,
        title=title,
        ...
        n_external_epochs=int(len(ext)),
        n_external_stars=n_ext_stars,
        ext_lit_pairs=ext_lit,
        ext_per_star=ext_per_star,
        ext_pipe_pairs=ext_pipe,
    ...
    if copy_key_table is not None:
        ...
        if summaries_glob and not ext_per_star.empty:
            ext_copy = copy_key_table.parent / "external_per_star_bias_rms.csv"
            ext_per_star.to_csv(ext_copy, index=False)
    ...
    return {
        ...
        "n_external_epochs": int(len(ext)),
        "n_external_stars": n_ext_stars,
        "n_external_lit_pairs": int(len(ext_lit)),
        "n_external_pipe_pairs": int(len(ext_pipe)),
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap = argparse.ArgumentParser(
        description="Compare pipeline / LAMOST / RAVE RVs to El-Badry literature master."
    )
    ap.add_argument(
        "--master",
        type=Path,
    ...
    ap.add_argument(
        "--summaries-glob",
        default=None,
        help="Optional glob of *_summary.txt for LAMOST/RAVE [EXTERNAL RV DATA] (08-full)",
    )
    ap.add_argument(
        "--external-sources",
        default=",".join(DEFAULT_EXTERNAL_SOURCES),
        help="Comma-separated telescope prefixes (default: LAMOST_LRS,LAMOST_MRS,RAVE_DR6)",
    )
    ap.add_argument(
        "--all-summary-external",
        action="store_true",
        help="Do not restrict external rows to Gaia ids present in the literature master",
    )
    ap.add_argument(
        "--report-dir",
        type=Path,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    external_sources = tuple(s.strip() for s in str(args.external_sources).split(",") if s.strip())
    ...
    summary = run_compare(
        ...
        external_sources=external_sources,
        restrict_external_to_literature=not args.all_summary_external,
    )
    tag = "literature_crosscheck_full" if args.summaries_glob else "literature_crosscheck_lite"
    print(
        f"{tag}: n_stars={summary['n_stars']} n_pairs={summary['n_pairs']} "
        f"n_external_stars={summary['n_external_stars']} "
        f"n_external_lit_pairs={summary['n_external_lit_pairs']} "
